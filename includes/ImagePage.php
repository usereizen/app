...
class ImagePage extends Article {
	...
	protected function imageMetadata($formattedMetadata) {
		...
		$out->addHTML( Xml::element( 'h2', [ 'id' => 'metadata' ], $context->msg( 'metadata' )->text() ) . "\n" );
	...
	protected function imageListing() {
		...
		$link = Html::element( 'a', [
			'id' => self::FILE_LINKS_SECTION_ID,
			'name' => self::FILE_LINKS_SECTION_ID,
			'rel' => 'nofollow',
		] );
		$out->addHTML("$link\n");
		# wikia change end
...
EOT
	...
	protected function uploadLinksBox() {
		...
		// Wikia change - @author: liz - adding class to ul and removed br
		$out->addHTML( "<ul class='mw-upload-links-box'>\n" );

		# "Upload a new version of this file" link
		$canUpload = $this->getTitle()->userCan( 'upload', $this->getContext()->getUser() );

		// Wikia change - @author: mech - replacing ->name with ->getName(), as File::$name is protected
		if ( $canUpload && UploadBase::userCanReUpload( $this->getContext()->getUser(), $this->mPage->getFile()->getName() ) ) {
